...
public var mouseRDownSignals : SignalSender;
public var mouseRUpSignals : SignalSender;
private var stateR : boolean = false;
...
function Update () {
	...
	if (state == false && joysticks[0].tapCount > 0) {
		...
		if (stateR == false && Input.GetMouseButtonDown (1)) {
			mouseRDownSignals.SendSignals (this);
			stateR = true;
		}
		
		else if (stateR == true && Input.GetMouseButtonUp (1)) {
			mouseRUpSignals.SendSignals (this);
			stateR = false;
		}
	#endif
#endif
